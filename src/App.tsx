...
function AppContent() {
  ...
  const handleFetch = async () => {
    if (!repoPath) return;
    try {
      await invoke("git_fetch_prune", { path: repoPath });
      loadCommits(repoPath); // Reload graph
      showAlert(
        "Fetch Successful",
  ...
  const handleCheckoutCommit = (hash: string) => {
    ...
    showConfirm(
      ...
      async () => {
        try {
          await invoke("git_checkout_commit", { path: repoPath, hash });
          loadCommits(repoPath);
  ...
  const handleReset = (hash: string, mode: 'soft' | 'mixed' | 'hard') => {
      ...
      showConfirm(
          ...
          async () => {
              try {
                  await invoke('git_reset', { path: repoPath, hash, mode: `--${mode}` });
                  loadCommits(repoPath);
  ...
  const handleRebase = (hash: string) => {
      ...
      showConfirm(
          ...
          async () => {
              try {
                  await invoke('git_rebase', { path: repoPath, branch: hash }); 
                  loadCommits(repoPath);
  ...
  const handleCherryPick = (hash: string) => {
    ...
    showConfirm(
      ...
      async () => {
        try {
          await invoke("git_cherry_pick", { path: repoPath, hash });
          loadCommits(repoPath);
        } catch (e: any) {
          setError(e.toString());
        }
  ...
  const handleRevert = (hash: string) => {
    ...
    showConfirm(
      ...
      async () => {
        try {
          await invoke("git_revert", { path: repoPath, hash });
          loadCommits(repoPath);
        } catch (e: any) {
          setError(e.toString());
        }
  ...
  const handleCreateTag = (hash: string) => {
    ...
    showInput("Create Tag", "Enter tag name:", async (name) => {
      if (!name) return;
      try {
        await invoke("git_tag_create", { path: repoPath, name, hash });
        loadCommits(repoPath);
      } catch (e: any) {
        setError(e.toString());
      }
  ...
  const handleMerge = (hash: string) => {
    ...
    showConfirm(
      ...
      async () => {
        try {
          await invoke("git_merge", { path: repoPath, branch: hash });
          loadCommits(repoPath);
        } catch (e: any) {
          setError(e.toString());
        }
  ...
  const handleCreateBranch = (hash: string) => {
    ...
    showInput("Create Branch", "Enter branch name:", async (name) => {
      if (!name) return;
      try {
        await invoke("git_branch_create", { path: repoPath, name, hash });
        loadCommits(repoPath);
        showAlert(
          "Success",
