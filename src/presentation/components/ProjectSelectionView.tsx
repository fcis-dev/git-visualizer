...
import { Folder, FolderPlus, GitBranch, Search, Trash2, ArrowRight, Settings } from 'lucide-react';
...
export function ProjectSelectionView({ onSelectRepo, onOpenSettings }: ProjectSelectionViewProps) {
  ...
  const loadFolders = async () => {
    ...
    try {
      ...
      const promises = loadedFolders.map(folder => loadReposForFolder(folder));
    ...
    <div className="flex-1 w-full flex flex-col h-full bg-slate-50 dark:bg-slate-950 p-8">
      <div className="max-w-5xl mx-auto w-full flex flex-col h-full">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100 mb-2">Projects</h1>
            <p className="text-slate-500 dark:text-slate-400">Select a repository to start working</p>
          </div>
        ...
        {loading && allRepos.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 overflow-y-auto pb-8 pr-2 custom-scrollbar">
            {filteredRepos.map(repo => (
              ...
              >
                ...
                <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100 mb-1 truncate" title={repo.name}>
                  {repo.name}
                </h3>
                <p className="text-sm text-slate-500 dark:text-slate-500 truncate mb-4" title={repo.path}>
                  {repo.path}
                </p>
                ...
                <div className="flex items-center space-x-2">
                   <div className="flex items-center space-x-1.5 px-2 py-1 bg-slate-100 dark:bg-slate-800 rounded text-xs font-medium text-slate-600 dark:text-slate-300">
                     ...
                     <span className="truncate max-w-[150px]">{repo.branch || '...'}</span>
                   </div>
                </div>
              </div>
            ))}
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-slate-400 dark:text-slate-600">
             ...
             <p className="text-sm max-w-xs text-center">
               {searchTerm ? "No projects match your search." : "Get started by adding a folder containing your Git repositories."}
             </p>
          </div>
        )}
